...
ifelse(fake_indicator == 1, suffix <- "randlabels_", suffix <- "")
...
ifelse(Cpar == "Cadj", suffix <- paste0(suffix, "Cadj_"), suffix <- paste0(suffix, ""))
...
### lisätty party fixed effect
R_session   <- sparse.model.matrix(~ 0 + year + party, data = speaker_metadata, verbose = FALSE)
...
saveRDS(utility_dem, file = paste0(output, "utility_dem_", suffix, "c0_partyf_", curdata))
saveRDS(utility_rep, file = paste0(output, "utility_rep_", suffix, "c0_partyf_", curdata))
saveRDS(utility, file = paste0(output, "utility_", suffix, "c0_partyf_", curdata))
saveRDS(phi, file = paste0(output, "phi_", suffix, "c0_partyf_", curdata))
